...
abstract contract BaseAdapter {
    ...
    /// principal that's set aside for Yield (e.g. 0.1e18 means that 10% of the principal is reserved).
    /// @notice If `0`, it means no principal is set aside for Yield
    ...
    /// digits using the following scheme: <onRedeemPrincipal(y/n)><collect(y/n)><combine(y/n)><issue(y/n)>
    ...
    /// @notice Hook called whenever a user redeems Principal
