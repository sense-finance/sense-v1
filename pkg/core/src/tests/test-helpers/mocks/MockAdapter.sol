...
contract MockAdapter is CropAdapter {
    ...
    uint256 public onPrincipalRedeemCalls;
    ...
    function onPrincipalRedeem(
        uint256, /* uBal */
        uint256, /* mscale */
        uint256, /* maxscale */
        uint256 /* tBal */
    ) public virtual override {
        onPrincipalRedeemCalls++;
    }
    ...
    function doIssue(uint256 maturity, uint256 tBal) external {
        ...
        (address principal, , address yield, , , , , , ) = Divider(divider).series(address(this), maturity);
        MockToken(principal).transfer(msg.sender, MockToken(principal).balanceOf(address(this)));
        MockToken(yield).transfer(msg.sender, MockToken(yield).balanceOf(address(this)));
    }
    ...
    function doRedeemPrincipal(uint256 maturity, uint256 uBal) external {
        Divider(divider).redeemPrincipal(address(this), maturity, uBal);
    }
}
