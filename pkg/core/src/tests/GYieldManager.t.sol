...
import { GYieldManager } from "../modules/GYieldManager.sol";
...
contract GYieldManager is TestHelper {
    ...
    function testFuzzCantJoinIfNotEnoughYield(uint128 balance) public {
        uint256 maturity = getValidMaturity(2021, 10);
        (, address yield) = sponsorSampleSeries(address(alice), maturity);
        if (calculateAmountToIssue(balance) == 0) return;
        hevm.warp(block.timestamp + 1 days);
        bob.doApprove(address(yield), address(bob.gYieldManager()));
    ...
    function testFuzzCantJoinIfNotEnoughYieldAllowance(uint128 balance) public {
        ...
        uint256 yieldBalance = Yield(yield).balanceOf(address(bob));
    ...
    function testCantJoinAfterFirstGYieldNotEnoughTargetBalance() public {
        ...
        bbalance = bbalance - calculateExcess(bbalance, maturity, yield);
        bob.doIssue(address(adapter), maturity, bbalance);
        uint256 bobYieldBalance = Yield(yield).balanceOf(address(bob));
        bob.doApprove(address(yield), address(bob.gYieldManager()));
        bob.doJoin(address(adapter), maturity, bobYieldBalance);
        uint256 bobGyieldBalance = ERC20(bob.gYieldManager().gyields(address(yield))).balanceOf(address(bob));
        ...
        alice.doApprove(address(yield), address(bob.gYieldManager()));
        hevm.warp(block.timestamp + 20 days);
        uint256 aliceYieldBalance = Yield(yield).balanceOf(address(alice));
        alice.doCollect(address(yield));
    ...
    // function testFuzzJoinFirstGYield(uint128 balance) public {
    ...
    //     uint256 gyieldBalance = ERC20(bob.gYieldManager().gyields(address(yield))).balanceOf(address(bob));
    //     assertEq(gyieldBalance, yieldBalance);
    // }

    // TODO: re-add this test once we use glcaims again
    // function testJoinAfterFirstGYield(uint128 balance) public {
    //     uint256 maturity = getValidMaturity(2021, 10);
    //     (, address yield) = sponsorSampleSeries(address(alice), maturity);
    //     uint256 yieldBaseUnit = 10**Yield(yield).decimals();
    ...
    //     alice.doApprove(address(yield), address(bob.gYieldManager()));
    //     alice.doApprove(address(target), address(bob.gYieldManager()));
    //     uint256 aliceYieldBalance = Yield(yield).balanceOf(address(alice));
    ...
    //     uint256 aliceGyieldBalance = ERC20(bob.gYieldManager().gyields(address(yield))).balanceOf(address(alice));
    //     assertEq(aliceGyieldBalance, aliceYieldBalance);
    //     assertEq(aliceTargetBalAfter, aliceTargetBalBefore);
    // }

    // TODO: re-add this test once we use glcaims again
    // function testJoinAfterFirstGYieldWithdrawsGap(uint128 balance) public {
    //     uint256 maturity = getValidMaturity(2021, 10);
    //     (, address yield) = sponsorSampleSeries(address(alice), maturity);
    //     uint256 yieldBaseUnit = 10**Yield(yield).decimals();
    ...
    //     hevm.warp(block.timestamp + 1 days);
    //     adapter.scale();
    //     uint256 balanceMinusExcess = uint128(balance - calculateExcess(balance, maturity, yield));
    //     target.balanceOf(address(alice));
    //     alice.doIssue(address(adapter), maturity, balanceMinusExcess);
    //     alice.doApprove(address(yield), address(alice.gYieldManager()));
    //     alice.doApprove(address(target), address(alice.gYieldManager()));
    //     uint256 aliceYieldBalance = Yield(yield).balanceOf(address(alice));
    //     uint256 aliceTargetBalBefore = target.balanceOf(address(alice));
    //     alice.doJoin(address(adapter), maturity, aliceYieldBalance);
    //     (, uint256 currScale) = adapter.lscale();
    //     uint256 initScale = alice.gYieldManager().inits(address(yield));
    //     uint256 gap = (aliceYieldBalance * currScale) / (currScale - initScale) / 10**18;
    //     uint256 aliceTargetBalAfter = target.balanceOf(address(alice));
    //     uint256 aliceGyieldBalance = ERC20(alice.gYieldManager().gyields(address(yield))).balanceOf(address(alice));
    //     assertEq(aliceGyieldBalance, aliceYieldBalance);
    ...
    function testFuzzExitFirstGYield(uint128 balance) public {
        balance = 100;
        // creating new periphery as the one from test helper already had a first gyield call
        Periphery newPeriphery = new Periphery(
            address(divider),
        ...
        bob.doIssue(address(adapter), maturity, balance);
        bob.doApprove(address(yield), address(bob.gYieldManager()));
        uint256 yieldBalanceBefore = Yield(yield).balanceOf(address(bob));
        bob.doJoin(address(adapter), maturity, yieldBalanceBefore);
        uint256 tBalanceBefore = target.balanceOf(address(bob));
        uint256 gyieldBalanceBefore = ERC20(bob.gYieldManager().gyields(address(yield))).balanceOf(address(bob));
        bob.doExit(address(adapter), maturity, gyieldBalanceBefore);
        uint256 gyieldBalanceAfter = ERC20(bob.gYieldManager().gyields(address(yield))).balanceOf(address(bob));
        uint256 yieldBalanceAfter = Yield(yield).balanceOf(address(bob));
    ...
    // TODO: re-add this test once we use glcaims again
    // function testFuzzExitGYieldWithCollected(uint128 balance) public {
    ...
    //     if (calculateAmountToIssue(balance) == 0) return;
    //     hevm.warp(block.timestamp + 1 days);
    //     balance = balance - uint128(calculateExcess(balance, maturity, yield));
    //     bob.doIssue(address(adapter), maturity, balance);
    //     bob.doApprove(address(yield), address(bob.gYieldManager()));
    //     hevm.warp(block.timestamp + 1 days);
    //     uint256 tBalanceBefore = target.balanceOf(address(bob));
    ...
    // TODO: re-add this test once we use glcaims again
    // function testExitAfterFirstGYield(uint128 balance) public {
    //     uint256 maturity = getValidMaturity(2021, 10);
    //     (, address yield) = sponsorSampleSeries(address(alice), maturity);
    //     // avoid fuzz tests in which nothing is issued
    //     if (calculateAmountToIssue(balance) == 0) return;
    ...
    //     uint256 gyieldBalanceAfter = ERC20(bob.gYieldManager().gyields(address(yield))).balanceOf(address(alice));
    //     uint256 yieldBalanceAfter = Yield(yield).balanceOf(address(alice));
