...
contract Adapters is TestHelper {
    ...
    function testDistributionSimple() public {
        ...
        alice.doCombine(address(adapter), maturity, ERC20(yield).balanceOf(address(alice)));
        assertClose(ERC20(reward).balanceOf(address(alice)), 10 * 1e18);
    ...
    function testDistributionCollectAndTransferMultiStep() public {
        ...
        bob.doTransfer(yield, address(jim), ERC20(yield).balanceOf(address(bob)));
        ...
        // now if another airdop happens, jim should get shares proportional to his new yield balance
        hevm.warp(block.timestamp + 1 days);
