...
const { SENSE_MULTISIG, OLYMPUS_POOL_PARTY } = require("../../hardhat.addresses");
...
task(
  ...
  for (let factory of mainnet.factories) {
    ...
    for (let t of targets) {
      ...
      const adapterAddress = await periphery.callStatic.deployAdapter(factoryAddress, t.address, ethers.utils.defaultAbiCoder.encode(["address"], [OLYMPUS_POOL_PARTY.get(chainId)]));
      await (await periphery.deployAdapter(factoryAddress, t.address, ethers.utils.defaultAbiCoder.encode(["address"], [OLYMPUS_POOL_PARTY.get(chainId)]))).wait();
      console.log(`${t.name} adapterAddress: ${adapterAddress}`);

      // Only sponsor Series of fork, otherwise we will deploy them through Defender
      if (chainId === "111") {
        console.log("\n-------------------------------------------------------");
        console.log("Checking multisig txs by impersonating the address");
