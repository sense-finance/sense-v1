...
task(
  ...
  if (chainId === "111") {
    ...
    // });
    // let signer = await hre.ethers.getSigner(senseAdminMultisigAddress);
    ...
    // await hre.network.provider.request({
    //   method: "hardhat_impersonateAccount",
    //   params: [poolManager.address],
    // });

    // signer = await hre.ethers.getSigner(poolManager.address);

    await network.provider.send("hardhat_setBalance", [
      poolManager.address,
      ethers.utils.parseEther("100000000000000000000").toHexString(),
    ]);

    const ptOracle = new ethers.Contract(
      ptOracleAddress,
      [
        "function setTwapPeriod(uint256) external",
    ...
    const lpOracle = new ethers.Contract(
      lpOracleAddress,
      ["function setTwapPeriod(uint256 _twapPeriod) external", "function twapPeriod() external view returns (uint256)"],
      signer,
    );
    ...
    console.log("\nPrevious PT oracle twap period:", (await lpOracle.twapPeriod()).toString());
    console.log(await poolManager.lpOracle(), setTwapData);
    console.log(await poolManager.callStatic.execute(lpOracle.address, 0, setTwapData, "9223372036813485055"));
    console.log("New PT oracle twap period:", (await lpOracle.twapPeriod()).toString());

    // console.log("\nPrevious PT oracle twap period:", (await ptOracle.twapPeriod()).toString());
    // await ptOracle.setTwapPeriod(19800).then(t => t.wait());
