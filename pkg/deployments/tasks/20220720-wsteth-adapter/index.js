...
task("20220720-wsteth-adapter", "Deploys wstETH adapter (with stETH as underlying)").setAction(
  async (_, { ethers }) => {
    ...
    for (let adapter of mainnet.adapters) {
      ...
      if (chainId !== "111") {
        console.log("\n-------------------------------------------------------");
        await verifyOnEtherscan("0x29872d0a2672bA73D540c53330E1A073c3cE2673", [
          divider.address,
          target.address,
          ifee,
          adapterParams,
