...
task(
  ...
  if (chainId === "5") {
    ...
    for (let adapter of goerli.adapters) {
      let { contractName, divider, target, ifee, adapterParams, reward } = adapter;
      ...
      console.log(`\nOnboard ${contractName} into Divider permissionlessly via Periphery`);
      if (!(await periphery.isTrusted(deployer))) {
        await (await periphery.onboardAdapter(adapterAddress, true)).wait();
      } else {
        throw Error(
          "You are trying to onboard the adapter with the trusted address. Try it out with a non trusted address.",
        );
      }
      ...
      console.log("\n-------------------------------------------------------");
      await verifyOnEtherscan(adapterAddress, [
        divider,
        target.address,
        await target.underlying(),
        ifee,
        adapterParams,
        reward,
      ]);
