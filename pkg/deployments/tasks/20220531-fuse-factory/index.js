...
task("20220531-fuse-factory", "Deploys Fuse Factory and adds it to the Periphery").setAction(
  async (_, { ethers }) => {
    ...
    for (let factory of mainnet.factories) {
      ...
      if (chainId === CHAINS.HARDHAT) {
        ...
        for (let t of targets) {
          ...
          const adapterAddress = await periphery.callStatic.deployAdapter(
            factoryAddress,
            t.address,
            ethers.utils.defaultAbiCoder.encode(["address"], [t.comptroller]),
          );
          await (
            await periphery.deployAdapter(
              factoryAddress,
              t.address,
              ethers.utils.defaultAbiCoder.encode(["address"], [t.comptroller]),
            )
