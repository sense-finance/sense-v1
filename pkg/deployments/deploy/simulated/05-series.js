  ...
  for (let factory of global.dev.FACTORIES) {
    for (let t of factory(chainId).targets) {
      ...
      for (let seriesMaturity of series) {
        ...
        const { principal: principalAddress, yield: yieldAddress } = await periphery.callStatic.sponsorSeries(
          adapter.address,
          seriesMaturity,
          true
        );
        ...
        log("Have the deployer issue the first 1,000,000 Target worth of Principal/Yield for this Series");
        ...
        const principal = new ethers.Contract(principalAddress, tokenAbi, signer);
        const yield = new ethers.Contract(yieldAddress, tokenAbi, signer);

        const principalBalance = await principal.balanceOf(deployer);
        ...
        log("Sending Principal to mock Balancer Vault");
        await principal.approve(balancerVault.address, ethers.constants.MaxUint256).then(tx => tx.wait());
        ...
        const { _principali, _targeti } = await pool.getIndices();
        const initialBalances = [null, null];
        initialBalances[_principali] = 0;
        ...
        log("Making swap to init Principal");
        await balancerVault
          .swap(
            {
              poolId,
              kind: 0, // given in
              assetIn: principal.address,
        ...
        log(
          `
            ...
            Principal price in Target: ${principalPriceInTarget}
            Principal price in Underlying: ${principalPriceInUnderlying}
          `,
        );
        ...
        await principal.approve(periphery.address, ethers.constants.MaxUint256).then(tx => tx.wait());
        await target.approve(periphery.address, ethers.constants.MaxUint256).then(tx => tx.wait());
        await yield.approve(periphery.address, ethers.constants.MaxUint256).then(tx => tx.wait());
        await pool.approve(periphery.address, ethers.constants.MaxUint256).then(tx => tx.wait());

        log("swapping target for principal");
        await periphery
          .swapTargetForPrincipal(adapter.address, seriesMaturity, ethers.utils.parseEther("1"), 0)
          .then(tx => tx.wait());

        log("swapping target for yields");
        await periphery
          .swapTargetForYield(adapter.address, seriesMaturity, ethers.utils.parseEther("1"), 0)
          .then(tx => tx.wait());

        log("swapping principal for target");
        await principal.approve(periphery.address, ethers.constants.MaxUint256).then(tx => tx.wait());
        await periphery
          .swapPrincipalForTarget(adapter.address, seriesMaturity, ethers.utils.parseEther("0.5"), 0)
          .then(tx => tx.wait());

        log("swapping yields for target");
        await periphery
          .swapYieldForTarget(adapter.address, seriesMaturity, ethers.utils.parseEther("0.5"))
          .then(tx => tx.wait());
