...
contract Space is IMinimalSwapInfoPool, BalancerPoolToken {
    ...
    function onJoinPool(
        ...
        (uint256[] memory _reqAmountsIn, uint256[] memory _dueProtocolFeeAmounts) = abi.decode(
            _userData,
            (uint256[], uint256[])
        );
    ...
    function _determineMaxJoin(
        ...
        (uint8 _zeroi, uint8 _targeti) = getIndices();
        (uint256 _zeroReserves, uint256 _targetReserves) = (_currentBalances[_zeroi], _currentBalances[_targeti]);
    ...
    function onSwap(
        ...
        if (request.kind == IVault.SwapKind.GIVEN_IN) {
            ...
            uint256 amountOut = _onSwapGivenIn(zeroIn, request.amount, balanceTokenIn, balanceTokenOut);
            ...
            uint256 amountIn = _onSwapGivenOut(zeroIn, request.amount, balanceTokenIn, balanceTokenOut);
