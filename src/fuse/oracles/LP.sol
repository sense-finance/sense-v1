...
contract LPOracle is PriceOracle, Trust {
    ...
    function _price(address _pool) internal view returns (uint256) {
        ...
        uint256 value = PriceOracle(msg.sender).price(tokenA).fmul(balanceA, FixedMath.WAD) + 
            PriceOracle(msg.sender).price(tokenB).fmul(balanceB, FixedMath.WAD);

        // price per lp token = pool value / total supply of lp tokens
        return value.fdiv(totalSupply, FixedMath.WAD);
    }
}